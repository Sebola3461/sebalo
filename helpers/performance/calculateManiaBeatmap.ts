...
export default async (
	...
	scores.forEach((score) => {
		let score_rate = 1;
		if (DifficultyAttributes.mods.acronyms.includes("EZ"))
			score_rate *= 0.5;
		if (DifficultyAttributes.mods.acronyms.includes("NF"))
			score_rate *= 0.5;
		if (DifficultyAttributes.mods.acronyms.includes("HT"))
			score_rate *= 0.5;

		const real_score = score / score_rate;

		let hit300_window =
			34 + 3 * Math.min(10, Math.max(0, 10 - beatmap.accuracy));
		let strain_value =
			((5 * Math.max(1, DifficultyAttributes.starRating / 0.2) - 4) **
				2.2 /
				135) *
		...
		const pp =
			(strain_value ** 1.1 + acc_value ** 1.1) ** (1 / 1.1) *
			pp_multiplier;
